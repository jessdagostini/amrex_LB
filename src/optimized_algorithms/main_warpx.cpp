...
void lb_real_data(string file, amrex::BoxArray &ba, int nnodes, int nranks, int nruns, int ranks_per_node) {
    ...
    for(int run_id = 0; run_id <= nruns; run_id++) {
        ...
        original_efficiency = static_cast<amrex::Real>(total_weight) / (nranks * max_load);

        amrex::Print() << "Original efficiency: " << original_efficiency << endl;
        metric_utils_add(MetricUtilsAlgorithms::ORIGINAL, MetricUtilsMetrics::USED_EFFICIENCY, original_efficiency, run_id);
        ...
        amrex::Real k_eff = 0.0, kk_eff = 0.0, s_eff = 0.0, sfc_eff = 0.0, knapsack_eff = 0.0, sfc_knapsack_eff = 0.0, sfc_painter_eff = 0.0, painter_knapsack_eff = 0.0;
        ...
        if (run_id > 0) {
            ...
            amrex::Long max_load = *max_element(rank_loads.begin(), rank_loads.end());
            knapsack_eff = static_cast<amrex::Real>(total_weight) / (nranks * max_load);

            amrex::Print() << "Knapsack previous efficiency: " << knapsack_eff << endl;
            metric_utils_add(MetricUtilsAlgorithms::KNAPSACK, MetricUtilsMetrics::PREVIOUS_EFFICIENCY, knapsack_eff, run_id);

        }
        
        time_start = amrex::second();
        knapsack_map = KnapSackDoIt(wgts, nranks, k_eff, true, nmax, false, false, run_id, bytes);
        amrex::Real ratio = k_eff / knapsack_eff;
        amrex::Print() << "Knapsack ratio: " << ratio << endl;
        if (ratio > 1.1) {
            prev_knapsack_map = knapsack_map; // Update previous knapsack map only if the ratio is significant
            metric_utils_add(MetricUtilsAlgorithms::KNAPSACK, MetricUtilsMetrics::USED_EFFICIENCY, k_eff, run_id);
        } else {
            metric_utils_add(MetricUtilsAlgorithms::KNAPSACK, MetricUtilsMetrics::USED_EFFICIENCY, knapsack_eff, run_id);
        }
        ...
        //     for (int j = 0; j < wgts.size(); ++j) {
        //         rank_loads[kk_map[j]] += wgts[j];
        //         total_weight += wgts[j];
        //     }
        //     amrex::Long max_load = *max_element(rank_loads.begin(), rank_loads.end());
        //     kk_eff = static_cast<amrex::Real>(total_weight) / (nranks * max_load);

        //     amrex::Print() << "Karmarkar-Karp previous efficiency: " << kk_eff << endl;
        //     metric_utils_add(MetricUtilsAlgorithms::KARMARKAR_KARP, MetricUtilsMetrics::PREVIOUS_EFFICIENCY, kk_eff, run_id);
        ...
        //     for (int j = 0; j < wgts.size(); ++j) {
        //         rank_loads[sfc_map[j]] += wgts[j];
        //         total_weight += wgts[j];
        //     }
        //     amrex::Long max_load = *max_element(rank_loads.begin(), rank_loads.end());
        //     sfc_eff = static_cast<amrex::Real>(total_weight) / (nranks * max_load);

        //     amrex::Print() << "SFC previous efficiency: " << sfc_eff << endl;
        //     metric_utils_add(MetricUtilsAlgorithms::SFC, MetricUtilsMetrics::PREVIOUS_EFFICIENCY, sfc_eff, run_id);
        // }

        // time_start = amrex::second();
        // sfc_map = SFCProcessorMapDoIt(ba, wgts, nranks, &s_eff, node_size, false, false, run_id, bytes);
        ...
        //     for (int j = 0; j < wgts.size(); ++j) {
        //         rank_loads[sfc_knapsack_map[j]] += wgts[j];
        //         total_weight += wgts[j];
        //     }
        //     amrex::Long max_load = *max_element(rank_loads.begin(), rank_loads.end());
        //     sfc_knapsack_eff = static_cast<amrex::Real>(total_weight) / (nranks * max_load);

        //     amrex::Print() << "SFC+Knapsack previous efficiency: " << sfc_knapsack_eff << endl;
        //     metric_utils_add(MetricUtilsAlgorithms::SFC_KNAPSACK, MetricUtilsMetrics::PREVIOUS_EFFICIENCY, sfc_knapsack_eff, run_id);
        // }

        // time_start = amrex::second();
        // sfc_knapsack_map = SFCProcessorMapDoItCombined(ba, wgts, nnodes, ranks_per_node, &sfc_eff, &knapsack_eff, false, false, run_id, bytes);
        ...
        //     for (int j = 0; j < wgts.size(); ++j) {
        //         rank_loads[sfc_painter_map[j]] += wgts[j];
        //         total_weight += wgts[j];
        //     }
        //     amrex::Long max_load = *max_element(rank_loads.begin(), rank_loads.end());
        //     sfc_painter_eff = static_cast<amrex::Real>(total_weight) / (nranks * max_load);

        //     amrex::Print() << "SFC+Painter previous efficiency: " << sfc_painter_eff << endl;
        //     metric_utils_add(MetricUtilsAlgorithms::SFC_PAINTER, MetricUtilsMetrics::PREVIOUS_EFFICIENCY, sfc_painter_eff, run_id);
        // }

        // time_start = amrex::second();
        // sfc_painter_map = painterPartition(ba,wgts,nranks, run_id, true);
        ...
        //     for (int j = 0; j < wgts.size(); ++j) {
        //         rank_loads[painter_knapsack_map[j]] += wgts[j];
        //         total_weight += wgts[j];
        //     }
        //     amrex::Long max_load = *max_element(rank_loads.begin(), rank_loads.end());
        //     painter_knapsack_eff = static_cast<amrex::Real>(total_weight) / (nranks * max_load);

        //     amrex::Print() << "Painter+Knapsack previous efficiency: " << painter_knapsack_eff << endl;
        //     metric_utils_add(MetricUtilsAlgorithms::PAINTER_KNAPSACK, MetricUtilsMetrics::PREVIOUS_EFFICIENCY, painter_knapsack_eff, run_id);
        // }

        // time_start = amrex::second();
        // painter_knapsack_map = SFCProcessorMapDoItCombinedPainter(ba, wgts, nnodes, ranks_per_node, &sfc_eff, &knapsack_eff, false, false, run_id, bytes);
...
void lb_generated_data(amrex::BoxArray &ba, int nnodes, int nranks, int nruns, int ranks_per_node, amrex::Real mean, amrex::Real stdev) {
    ...
    for (int r = 1; r<=nruns; r++) {
        ...
        amrex::Real sfc_eff = 0.0, knapsack_eff = 0.0, kk_eff = 0.0, k_eff = 0.0, s_eff = 0.0;
        int node_size = 0;
        double time_start=0;
        ...
        time_start = amrex::second();
        vector<int> vec=painterPartition(ba,wgts,nranks, r);
        amrex::Print()<<" Final SFC+Painter time: " << amrex::second() - time_start << endl<<endl;
